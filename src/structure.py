...
def ransac_fundamental_matrix(p1, p2, iterations, T):
    """
    p1: points from image 1 of shape (3,N)
    p2: points from image 2 of shape (3,N)
    iterations: int num of iterations for ransac

    returns:
        E_best: essental matrix (3,3)
        in1_best: inlyers of p1 (3,M)
        in2_best: inlyers of p2 (3.M)
    """
    ...
    best_inlyers = 0
    num_matches = p1.shape[1]
    inlyers1 = []
    inlyers2 = []
    E_best = np.array([])

    for _ in range(iterations):
        sample = np.random.randint(0,num_matches, size=8)
        E = compute_fundamental_normalized(p1[:,sample],p2[:,sample])
        in1 = []
        in2 = []
        inlyers = 0
        for i in range(num_matches):
            temp = np.dot(p1[:,i],E.T) # dot homogenous point with E
            error = np.dot(temp, p2[:,i].T)
            if abs(error) < T:
                in1.append(p1[:,i])
                in2.append(p2[:,i])
                inlyers += 1

        if inlyers > best_inlyers:
            best_inlyers = inlyers
            E_best = E
            inlyers1 = in1
            inlyers2 = in2

    inlyers1 = np.array(inlyers1)
    inlyers2 = np.array(inlyers2)
    return E_best, inlyers1.T, inlyers2.T
